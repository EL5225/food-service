...
import { getAllSavedResep } from "../../utils";
...
export const Favorit = () => {
  ...
  const getDataResep = async (search, page) => {
    try {
      setLoading(false);
      const data = await getAllSavedResep({
        search,
        page,
      });
  ...
  return (
    <section className="flex flex-col w-full h-full lg:py-12 lg:px-16 pt-28 pb-12 px-10 gap-14 overflow-auto">
      ...
      <div className="w-full flex flex-col gap-10">
        <h1
          className={`${
            searchValue ? "lg:text-4xl text-2xl" : "lg:text-3xl text-xl"
          ...
          {searchValue ? (
            ...
            <TypeAnimation
              sequence={[100, `${message}`]}
              wrapper="span"
              cursor={true}
              speed={70}
            />
          ) : (
            <div className="flex flex-col items-center w-full h-[3rem] px-4 py-6 bg-gray-300 gap-3 rounded-lg shadow-md animate-pulse ease-out"></div>
          )}
        </h1>
