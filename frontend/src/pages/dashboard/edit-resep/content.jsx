...
import { getCategory, useCreateResep, useDeleteResep } from "../../../utils";
import { VSCreateResep } from "../../../libs/validation";

export const EditResepContent = () => {
  const [isLoading, setIsLoading] = useState(false);
  const [imageURL, setImageUrl] = useState(null);
  const [categories, setCategories] = useState([]);

  const { update, uploadImage } = useCreateResep();
  const { deleteImage } = useDeleteResep();
  ...
  const resepData = useAsyncValue();

  const navigate = useNavigate();

  const resep = useMemo(() => {
    return resepData?.data?.[0];
  });

  const [imageCards, setImageCards] = useState(resep?.resepImages);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      const newImages = imageCards.filter((item) => {
        return !item.id;
      });
  ...
  useEffect(() => {
    reset({
      categories_id: resep?.categories?.id,
      ...resep,
    });
  }, []);

  return (
    <section className="flex flex-col w-full h-full lg:py-12 lg:px-12 pt-28 pb-12 px-4 gap-14 overflow-auto">
      <h1 className="text-3xl font-bold">Tambah Resep</h1>
      <div className="w-full flex lg:flex-row flex-col xl:gap-0 lg:gap-12 gap-16 items-center">
        <div className="w-[45%] flex flex-col gap-12">
          <figure className="xl:w-[85%] lg:w-full flex items-center justify-center">
            <img
          ...
          <div className="flex flex-col items-center xl:w-[85%] w-full">
            <label
              className="flex lg:w-[60%] sm:w-full w-[80vw] items-center justify-center bg-black text-white p-4 text-base font-medium rounded-md cursor-pointer"
              htmlFor="image_url">
              Upload gambar
            </label>
            <input
          ...
          <div className="xl:w-[85%] w-full lg:flex flex-col gap-3 justify-center hidden">
            {imageCards?.map((item, index) => (
              <CardImage
                ...
                onDelete={async () => {
                  ...
                  if (item.id) {
                    const newImages = imageCards.find((image) => {
                      return item.id === image.id;
                    });
                    await deleteImage(resep?.id, newImages?.id);

                    setImageCards(
                      imageCards.filter((image) => {
                        return image.id !== newImages?.id;
                      })
                    );
                  } else {
        ...
        <div className="md:w-[70%] w-full flex flex-col gap-3 justify-center lg:hidden">
          {imageCards?.map((item, index) => (
            <CardImage
              key={index}
              name={item?.image_name}
        ...
        <FormProvider {...form}>
          <form
            onSubmit={onSubmit}
            className="lg:w-[45%] w-full flex flex-col gap-4">
            <TextField
              name="name"
              label="Nama Masakan"
